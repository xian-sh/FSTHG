...
from .utils import EEGSignalIO, MetaInfoIO

log = logging.getLogger('torcheeg')


def handle_remove_readonly(func, path, exc_info):
    # If the error is due to a permission error, change the file permission and retry
    if not os.access(path, os.W_OK):
        os.chmod(path, os.stat.S_IWUSR)
        func(path)
    else:
        raise


def remove_directory_with_retry(path, retries=5, delay=1):
        ...
        if not self.exist(self.io_path) or self.io_mode == 'memory':
            ...
            if self.num_worker == 0:
                try:
                    ...
                    # remove_directory_with_retry(self.io_path)
                    # 确保所有文件句柄都已关闭
                    worker_results = []  # 清空结果列表
                    import gc
                    gc.collect()  # 强制垃圾回收
                    ...
                    try:
                        remove_directory_with_retry(self.io_path, retries=10, delay=2)
                    except PermissionError:
                        log.warning(f"无法删除目录 {self.io_path}，请在程序退出后手动删除")
